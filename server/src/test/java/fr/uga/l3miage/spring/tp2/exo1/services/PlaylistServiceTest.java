...
class PlaylistServiceTest {
    ...
    void getPlaylist(){
        //given
        PlaylistEntity playlistEntity = PlaylistEntity.builder().name("test").description("Une description de test").songEntities(Set.of()).build();
        when(playlistComponent.getPlaylist("test")).thenReturn(playlistEntity);
        PlaylistResponseDTO responseExpected = playlistMapper.toResponse(playlistEntity);

        //when
        PlaylistResponseDTO response = playlistService.getPlaylist("test");

        // then
        assertThat(response).usingRecursiveComparison().isEqualTo(responseExpected);
        verify(playlistComponent,times(1)).getPlaylistEntity("test");
        verify(playlistMapper,times(1)).toResponse(playlistEntity);
    }
